/*
 * @Author: ben
 * @LastEditTime: 2022-01-13 16:18:23
 */

import { postDefaultProcess } from "@/api/flow";
import { UserModule } from "@/store/modules/user";
import { FlowDefine } from "@/components/rvue/custom/packages/flow/_core/FlowDefine";
...
export const defaultRequrest = function (this: Record<string, any>, formId: string) {
	let vm = this;
	let queryList = vm.form.type == "F" ? queryListF : queryListC;
	viewStructure["queryList"] = queryList;
	if (vm.form.type == "F") {
		postDefaultProcess({ appCode: vm.$route.params.appCode, formCode: formId, jsonNodeList: FlowDefine.defFlowData });
	}
	...
	// }).then((reslove: any) => {
	// 	let { data } = reslove;
	// 	let rightList = data.find((item: any) => item.type == "9");

	// 	console.log(reslove, "reslove");
	// 	updateRightList({
	// 		formCode: rightList.formCode,
	// 		code: rightList.code,
	// 		name: rightList.name,
	// 		viewStructure,
	// 		perms: [],
