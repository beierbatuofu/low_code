<template>
	<div class="rvue-empty">
		<div v-if="!src && !$slots.default" class="default-icon">
					...
					<g id="B-type" transform="translate(-1268.000000, -535.000000)">
						<g id="Group-2" transform="translate(1268.000000, 535.000000)">
							...
							<g id="Rectangle-Copy-17" transform="translate(53.000000, 45.000000)">
								...
								<use id="Mask" fill="#E0E3E9" transform="translate(8.500000, 18.000000) scale(-1, 1) translate(-8.500000, -18.000000) " :xlink:href="`#path-3-${id}`" />
								<polygon
									id="Rectangle-Copy"
									fill="#D5D7DE"
									:mask="`url(#mask-4-${id})`"
		...
		<img :src="src" v-if="!$slots.default" ondragstart="return false" />
		<slot v-if="$slots.desc" name="desc"></slot>
		<p v-else class="desc">{{ desc }}</p>
...
<script lang="ts">
	...
	@Component({})
	class RvueEmpty extends Vue {
		@Prop({ type: String, default: "" }) desc!: string;
		@Prop({ type: String, default: "" }) src!: string;
		private id = 1;
		public static install: (param: Vue) => any;
		public static compName = "rvue-empty";
	}
	RvueEmpty.install = function (Vue: any) {
		Vue.component(RvueEmpty.compName, RvueEmpty);
	};
	export default RvueEmpty;
</script>
