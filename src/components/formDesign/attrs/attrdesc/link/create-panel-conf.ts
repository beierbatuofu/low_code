...
export default function (editor: any, text: string, link: string): any {
	// panel 中需要用到的id
		...
		tabs: [
			{
				...
				tpl: `<div>
                        ...
                        <input
                            ...
                            class="block"
                            value="${link}"
                            placeholder="${editor.i18next.t("请插入如")}http://...或者 https://..."/>
                        </td>
				...
				events: [
					// 插入链接
					{
						...
						fn: () => {
							...
							let html: string = "";
							if (selection && !selection?.isCollapsed) html = insertHtml(selection, topNode)?.trim();
