...
export default {
	...
	methods: {
		initHistory(data) {
			if (data.storage) {
				...
				if (history) {
					...
					return this.$utils.deepClone(steps[index]);
				}
			}
			this.historySteps = {
				...this.historySteps,
		...
		handleHistoryChange(data) {
			...
			this.historySteps.steps[this.historySteps.index] = this.$utils.deepClone(data);
		...
		handleUndo() {
			...
			return this.$utils.deepClone(this.historySteps.steps[this.historySteps.index]);
		},
		handleRedo() {
			if (this.historySteps.index != this.historySteps.steps.length - 1)
