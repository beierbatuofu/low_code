<!--
 * @Author: ben
 * @LastEditTime: 2022-03-07 17:38:15
-->
<template>
	<div>
		<rvue-select :multiple="multiple" v-model="value" filterable :placeholder="placeholder" @change="handleChange" style="width: 100%">
			<rvue-option style="width: 350px" v-for="(item, index) in resetOptions" :key="index" :value="item.code || item.fieldId" :label="getName(item)">{{ getName(item) }}</rvue-option>
		</rvue-select>
	</div>
</template>
<script lang="ts">
	import { Vue, Component, Watch, Prop, Model, PropSync } from "vue-property-decorator";
	const types = [["input", "select", "radio", "textarea", "sn"],["address"], ["number"], ["datetime"], ["userRadio"], ["user"], ["dept"], ["deptRadio"], ["selectMulti", "checkbox"]];
